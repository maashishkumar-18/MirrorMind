...
class VectorStore:
    ...
    def upsert(self, result: EmbeddingBatchResult, namespace: str = "default") -> UpsertResult:
        ...
        for batch_idx, i in enumerate(range(0, len(chunks), BATCH_SIZE)):
            ...
            if total_batches > 5 and (batch_idx + 1) % 5 == 0:
                print(
                    f"   Progress: {min(i + BATCH_SIZE, len(chunks))}/{len(chunks)} chunks processed"
                )
    ...
    def _prepare_vectors(self, chunks: list[EmbeddedChunk]) -> list[dict[str, Any]]:
        ...
        for chunk in chunks:
            ...
            vectors.append({"id": chunk.chunk_id, "values": chunk.embedding, "metadata": metadata})

        return vectors

    def _upsert_batch_with_stats(
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    print(
                        f"⚠️  Upsert failed (attempt {attempt + 1}): {e}. Retrying in {wait_time:.1f}s..."
                    )
        ...
        def _query_one(ns: str) -> list[dict[str, Any]]:
            try:
                results = self.index.query(
                    vector=vector, top_k=top_k, namespace=ns, filter=filter, include_metadata=True
                )
                return [
