...
class ConversationState:
    ...
    current_subconcept: str | None = None
    history: deque = field(default_factory=lambda: deque(maxlen=20))

    last_explained_concept: str | None = None
    last_compared_pair: tuple[str, str] | None = None
    ...
    def get_primary_concept(self) -> str | None:
        """Get most specific concept available."""
        return (
            self.current_subconcept
            or self.current_concept
            or self.current_topic
            or self.chapter
            or self.module
            or self.subject
        )
    ...
    def get_context_summary(self) -> str:
        ...
        if self.current_concept:
            parts.append(f"Concept: {self.current_concept}")
        if self.current_subconcept:
            parts.append(f"Subconcept: {self.current_subconcept}")
...
class QueryRewriter:
    ...
    INCOMPLETE_PATTERNS = [
        ...
        (
            re.compile(
                r"^(example|give example|show example|illustrate|demonstrate|sample)$",
                re.IGNORECASE,
            ),
            IntentType.EXAMPLE,
        ),
        ...
        (
            re.compile(r"^(summary|summarize|tldr|brief|in short|recap)$", re.IGNORECASE),
            IntentType.SUMMARY,
        ),
        (re.compile(r"^(formula|equation|math|calculation)$", re.IGNORECASE), IntentType.FORMULA),
        (
            re.compile(r"^(advantages|pros|benefits|merits|strengths)$", re.IGNORECASE),
            IntentType.ADVANTAGES,
        ),
        (
            re.compile(
                r"^(disadvantages|cons|drawbacks|demerits|limitations|weaknesses)$", re.IGNORECASE
            ),
            IntentType.DISADVANTAGES,
        ),
        (
            re.compile(r"^(applications|uses|applied|real world|practical|usage)$", re.IGNORECASE),
            IntentType.APPLICATIONS,
        ),
        (
            re.compile(r"^(definition|define|meaning|what is)$", re.IGNORECASE),
    ...
    FOLLOWUP_PATTERNS = [
        ...
        re.compile(
            r"^(example|give example|show example|illustrate|demonstrate|sample)$", re.IGNORECASE
        ),
        re.compile(r"^(difference|diff|compare|vs\.?|versus)$", re.IGNORECASE),
        re.compile(r"^(explain|elaborate|describe|tell me more|go on)$", re.IGNORECASE),
        re.compile(r"^(again|repeat|re-explain|explain again|once more)$", re.IGNORECASE),
    ...
    COMPARISON_REGEX = re.compile(
        r"compare\s+(.+?)\s+(?:and|vs\.?|versus|with)\s+(.+?)(?:\?|$|\.)", re.IGNORECASE
    )
    ...
    def __init__(self, technical_terms_path: str | None = None, max_cache_size: int = 1000):
        """
        ...
        self.pronoun_pattern = re.compile(r"\b(" + "|".join(self.pronouns) + r")\b", re.IGNORECASE)
    ...
    def _load_technical_terms(self, path: str | None) -> set[str]:
    ...
    def rewrite(self, query: str, state: ConversationState) -> tuple[str, dict[str, Any]]:
        ...
        if metadata["was_rewritten"]:
            logger.info(
                f"Rewritten: '{original}' → '{query}' (confidence: {metadata['rewrite_confidence']:.2f})"
            )
    ...
    def rewrite_for_dual_retrieval(
    ...
    def _resolve_pronouns(self, query: str, state: ConversationState) -> str:
        ...
        if concept:
            resolved = re.sub(
                r"\b" + re.escape(concept) + r"\s+it\b", concept, resolved, flags=re.IGNORECASE
            )
            resolved = re.sub(
                r"\b" + re.escape(concept) + r"\s+this\b", concept, resolved, flags=re.IGNORECASE
            )
    ...
    def _extract_comparison(self, query: str) -> str | None:
    ...
    def _detect_intent(self, query: str) -> IntentType | None:
        query_lower = query.lower()
    ...
    def _calculate_confidence(self, query: str, state: ConversationState, metadata: dict) -> float:
        ...
        features = {
            ...
            "pronoun_resolved": (
                1.0 if metadata["rewrite_type"] == RewriteType.PRONOUN_RESOLUTION else 0.0
            ),
...
if __name__ == "__main__":
    ...
    state.add_turn(
        "assistant", "Deadlock is a situation where...", topic="Concurrency", concept="Deadlock"
    )
