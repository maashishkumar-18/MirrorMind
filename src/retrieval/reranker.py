...
class RerankerConfig:
    ...
    @classmethod
    def from_yaml(cls, path: str | None = None) -> "RerankerConfig":
        """
...
class CrossEncoderReranker:
    ...
    def __init__(self, config: RerankerConfig):
        ...
        self._cache = LRUCache(
            max_size=config.max_cache_entries, ttl_seconds=config.cache_ttl_seconds
        )
        ...
        if self._model is None:
            try:
                ...
                self._model = CrossEncoder(
                    self.model_name, max_length=self.config.cross_encoder_max_length
                )
            except ImportError:
                raise ImportError(
    ...
    def rerank(
        ...
        pairs = [
            (query, c.get("content", "")[: self.config.cross_encoder_max_length])
            for c in candidates
        ]
    ...
    def _build_results(
        self, candidates: list[dict], scores: list[float], top_k: int
    ) -> list[RerankedChunk]:
        ...
        if self.config.normalize_scores:
            ...
            scored = [(c, n) for (c, _), n in zip(scored, normalized, strict=False)]
...
class LLMReranker:
    ...
    def __init__(self, config: RerankerConfig):
        self.config = config
        self.model_name = config.model_name or config.default_models.get("llm", "deepseek-chat")
...
class Reranker:
    ...
    def __init__(self, config: RerankerConfig | None = None, config_path: str | None = None):
        """
    ...
    def _hybrid_rerank(self, query: str, candidates: list[dict], top_k: int) -> RerankerResult:
        """
    ...
    def _parallel_hybrid_rerank(
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_parallel_workers) as executor:
            ...
            ce_future = executor.submit(ce_reranker.rerank, query, candidates, len(candidates))
            futures.append(("cross_encoder", ce_future))

            # Submit LLM task
            llm_future = executor.submit(llm_reranker.rerank, query, candidates, len(candidates))
    ...
    def _combine_hybrid_results(
        ...
        total_time = sum(r.get("processing_time", 0.0) for r in results_map.values())
        if not self.config.parallel_hybrid:
            total_time = sum(r.get("processing_time", 0.0) for r in results_map.values())
        else:
            # In parallel mode, total time is max of the times, not sum
            total_time = max(r.get("processing_time", 0.0) for r in results_map.values())
...
if __name__ == "__main__":
    ...
    print(
        f"   Model: {config.model_name or config.default_models.get(config.backend_type, 'default')}"
    )
    ...
    print(
        f"   Cache: {config.cache_enabled} (TTL: {config.cache_ttl_seconds}s, Max: {config.max_cache_entries})"
    )
    ...
    print("Query: 'What is deadlock and how to prevent it?'")
    ...
    try:
        ...
        result = reranker.rerank(
            query="What is deadlock and how to prevent it?", candidates=sample_candidates
        )
    ...
    for var in [
        "RAGPIPE_RERANKER_BACKEND",
        "RAGPIPE_RERANKER_MODEL",
        "RAGPIPE_RERANKER_INPUT_K",
        "RAGPIPE_RERANKER_OUTPUT_K",
        "RAGPIPE_RERANKER_MIN_SCORE",
        "RAGPIPE_RERANKER_BATCH_SIZE",
