...
class ConfidenceConfig:
    ...
    @classmethod
    def from_yaml(cls, path: str | None = None) -> "ConfidenceConfig":
        """
        ...
        return cls(
            ...
            semantic_coverage=SemanticCoverageConfig(
                ...
                embedding_similarity_threshold=float(
                    semantic_data.get("embedding_similarity_threshold", 0.7)
                ),
            ...
            retry=RetryConfig(
                ...
                enable_aggressive_expansion=bool(
                    retry_data.get("enable_aggressive_expansion", True)
                ),
            ),
...
class WeightOptimizer:
    ...
    def __init__(
        self, config: LearnedWeightsConfig, feature_names: list[str], initial_weights: np.ndarray
    ):
        """
    ...
    def add_sample(
        ...
        true_level: ConfidenceLevel | None = None,
    ) -> None:
        """
        ...
        try:
            ...
            if weights_dict:
                self.weights = np.array(
                    [weights_dict.get(name, 0.0) for name in self.feature_names]
                )
                # Ensure sum to 1
                self.weights = self.weights / np.sum(self.weights)
                return True
...
class ConfidenceScorer:
    ...
    def __init__(self, config: ConfidenceConfig | None = None, config_path: str | None = None):
        """
        ...
        self._base_weights = np.array(
            [getattr(self.config.feature_weights, name) for name in self._feature_names]
        )
        ...
        if self.config.learned_weights.enabled:
            ...
            if self.config.learned_weights.weights_path:
                self.weight_optimizer.load_weights(self.config.learned_weights.weights_path)
    ...
    def get_feature_weights(self) -> dict[str, float]:
        ...
        if self.weight_optimizer and self.config.learned_weights.enabled:
            ...
            return {
                name: float(weight)
                for name, weight in zip(self._feature_names, self._base_weights, strict=False)
            }
    ...
    def evaluate(
        ...
        feature_scores["score_distribution"] = self._evaluate_score_distribution(reranker_result)

        # 3. Chunk diversity (coverage across sources)
        feature_scores["chunk_diversity"] = self._evaluate_chunk_diversity(assembled_context)
        ...
        feature_scores["metadata_match"] = self._evaluate_metadata_match(metadata_filters_applied)
        ...
        weighted_score = sum(weights[name] * feature_scores[name] for name in self._feature_names)
        ...
        if (
            weighted_score < self.config.thresholds.high
            and weighted_score >= self.config.thresholds.medium
        ):
            if feature_scores["semantic_coverage"] < 0.5:
                warnings.append(
                    "Low semantic coverage — query terms not well represented in context"
                )
    ...
    def _evaluate_reranker_scores(
        self, context: AssembledContext, reranker_result: RerankerResult
    ) -> float:
    ...
    def _evaluate_score_distribution(self, reranker_result: RerankerResult) -> float:
        ...
        max_drop = max(scores[i] - scores[i + 1] for i in range(len(scores) - 1))
        ...
        # Flat distribution → low confidence (uncertain relevance)
            ...
            range_size = (
                self.config.score_distribution_steep_drop - self.config.score_distribution_flat
            )
            if range_size > 0:
    ...
    def _evaluate_semantic_coverage(
        self, query: str, keywords: str, context: AssembledContext
    ) -> float:
        """Evaluate how well query terms are covered in the context."""
        if not context.formatted_context:
            return 0.0

        return self.semantic_scorer.score_against_terms(query, keywords, context.formatted_context)
    ...
    def _evaluate_retrieval_depth(
        self, sources_used: list[str], reranker_result: RerankerResult
    ) -> float:
        """
        ...
        if level == ConfidenceLevel.HIGH:
            ...
            if feature_scores.get("reranker_scores", 0) < 0.6:
                recommendations.append(
                    "Top chunks have moderate relevance — verify with user if unsure"
                )
            ...
            if retry_count < self.config.retry.max_retries:
                ...
                recommendations.append(
                    f"Retrying with expanded parameters (attempt {retry_count + 1})"
                )
            else:
                ...
                if feature_scores.get("semantic_coverage", 0) < 0.2:
                    ...
                    recommendations.append(
                        "Consider asking the user to rephrase or clarify their question"
                    )
...
if __name__ == "__main__":
    ...
    print(
        f"   Semantic coverage: keywords={config.semantic_coverage.use_keywords}, "
