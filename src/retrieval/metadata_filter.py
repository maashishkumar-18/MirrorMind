...
class MetadataFilterConfig:
    ...
    field_mapping_source: str | None = None  # Path to ingestion schema

    @classmethod
    def from_yaml(cls, path: str | None = None) -> "MetadataFilterConfig":
        """
...
class MetadataFilterBuilder:
    ...
    def __init__(
        ...
        ingestion_schema_path: str | None = None,
    ...
    def build(
        ...
        if conversation_state:
            context_filter, context_applied, ctx_warnings = self._build_from_conversation(
                conversation_state
            )
            if context_filter:
    ...
    def build_simple(
        ...
        if not self.config.validate_operator(resolved_field, operator):
            validation_errors.append(f"Invalid operator '{operator}' for field '{resolved_field}'")
    ...
    def build_compound(
        ...
        if not filters:
            return FilterResult(filter_dict={}, applied_filters=[], is_empty=True)
    ...
    def get_field_schema(self, field_name: str) -> dict | None:
    ...
    def list_fields(self) -> list[dict[str, Any]]:
        ...
        return [{"name": name, **schema.to_dict()} for name, schema in self.config.fields.items()]
    ...
    def _build_from_dict(
        self, filter_dict: dict[str, Any], strict_validation: bool = False
    ) -> tuple:
        """
            ...
            if isinstance(value, dict):
                ...
                for operator, val in value.items():
                    if not self.config.validate_operator(resolved, operator):
                        errors.append(f"Invalid operator '{operator}' for field '{resolved}'")
    ...
    def _build_from_conversation(self, state: ConversationState) -> tuple:
        """
...
def create_ingestion_schema_example():
    ...
    return {
        "metadata": {
            ...
            "page_end": {"type": "number", "description": "Ending page number", "required": False},
            "is_primary": {
                "type": "boolean",
...
if __name__ == "__main__":
    ...
    print("   Input: field='topic', value='Deadlock'")
    ...
    print("   Input: field='course', value='Operating Systems'")
    print(f"   Output: {result.filter_dict}")
    print(f"   Applied: {result.applied_filters}")
    ...
    print("   Input: field='page_start', value=5, operator='$gte'")
    print(f"   Output: {result.filter_dict}")
    ...
    print("   Input: field='page_start', value='five', operator='$gte'")
    print(f"   Output: {result.filter_dict}")
    ...
    print("   Input: [topic=Deadlock, page_start=5, page_end='ten']")
    print(f"   Output: {result.filter_dict}")
    print(f"   Validation errors: {result.validation_errors}")
    print(f"   Warnings: {result.warnings}")
    ...
    pipeline_config = PipelineConfig(name="test", metadata_filters={})

    result = builder.build(pipeline_config, conversation_state=state)
    print("   Conversation: CS → Process Management → Concurrency → Deadlock")
    print(f"   Output: {result.filter_dict}")
    print(f"   Applied: {result.applied_filters}")
