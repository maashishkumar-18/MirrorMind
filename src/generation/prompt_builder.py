...
class ExamAnswerOrderingClassifier:
    ...
    def __init__(self):
        ...
        self._section_order = ["thesis", "evidence", "examples", "conclusion"]
    ...
    def classify(self, chunk: RetrievedChunk) -> str:
        ...
        if any(
            phrase in content for phrase in ["according to", "data shows", "research indicates"]
        ):
            return "evidence"
...
class ChronologicalContextAssembly(ContextAssemblyStrategy):
    ...
    def assemble(
        ...
        sorted_chunks = sorted(chunks, key=lambda c: c.metadata.get("position", 0))
...
class ContextAssembler:
    ...
    def __init__(
        ...
        strategy: ContextAssemblyStrategy | None = None,
    ):
        """
    ...
    def assemble(
        self, chunks: list[RetrievedChunk], mode_config: ModeConfig, max_tokens: int
    ) -> tuple[str, int, list[str]]:
        """
        Assemble chunks into a formatted context string.
...
class PromptBuilder:
    ...
    def build(self, request: GenerationRequest, mode_config: ModeConfig) -> Prompt:
        ...
        if mode_config.include_context_header and request.chunks:
            ...
            context_header = (
                f"Context from: {first_chunk.course_name} - {first_chunk.chapter_title}"
            )
...
def create_standard_assembler(
    classifier: SectionClassifier | None = None, section_headers: dict[str, str] | None = None
) -> ContextAssembler:
...
def create_exam_assembler(classifier: SectionClassifier | None = None) -> ContextAssembler:
...
if __name__ == "__main__":
    ...
    from src.generation.config import AnswerFormat, CitationStyle, ModeConfig
    ...
    for strategy_name, strategy in strategies.items():
        ...
        print(
            f"Tokens - Context: {prompt.context_token_count}, "
    ...
    variables = {"context": "Sample context", "course": "Sample course"}
